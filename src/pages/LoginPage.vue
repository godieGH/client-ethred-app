   ...
   <q-page class="flex flex-center bg-gradient">
      <q-card class="q-pa-lg shadow-2" style="width: 400px; max-width: 85vw; border-radius: 10px">
         ...
         <q-form @submit="submitForm" ref="loginForm" class="q-gutter-md">
            ...
            <router-link flat to="/forgot-password" :class="routerLinkColor">Forgot Password?</router-link>
         </div>

            <div class="row justify-between items-center q-mt-md">
               <q-btn label="Login" type="submit" :color="btnColor" :loading="loading"
                  spinner-size="24px"
...
<script setup>
   ...
   const submitForm = async () => {
      ...
      if (valid) {
         ...
         try {
            const res = await axios.post(`${baseUrl}/users/login`, credentials.value)
            if(res.status === 200) {
            ...
            if(err.status === 401) {
               ...
               msg.value = {
                  ...
                  body: "Please check you emails and verify it is you."
               }
            } else {
